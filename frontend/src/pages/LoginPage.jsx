...
export default function LoginPage() {
  ...
  const avatarUrl = `https://api.dicebear.com/8.x/avataaars/svg?seed=${encodeURIComponent(displayName || 'Guest')}`;
  ...
  const handleGuest = async (e) => {
    ...
    setLoading(true);
    try {
      await guestLogin(displayName.trim());
  ...
  const handleEmail = async (e) => {
    ...
    try {
      ...
      if (mode === 'register') {
        ...
        data = await userApi.register(displayName.trim(), email, password);
      } else {
  ...
  const handleGoogle = () => {
    if (!GOOGLE_AVAILABLE) {
      setError('Google login not configured. Use email or guest.');
      return;
    }
    // Supabase handles the full OAuth flow
    ...
    <div className="min-h-screen bg-void flex items-center justify-center px-4">
      ...
      <div className="relative z-10 w-full max-w-sm animate-slide-up">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sub text-sm mt-1">Sign in to watch together</p>
        </div>
        ...
        <div className="card p-6">
          ...
          {mode === 'guest' && (
            <form onSubmit={handleGuest} className="space-y-4">
              <div className="flex items-center gap-3 pb-4 border-b border-border">
                <div className="relative shrink-0">
                  <img src={avatarUrl} alt="avatar"
                       className="w-12 h-12 rounded-full border-2 border-amber/30 bg-raised" />
                  <div className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5
                                   rounded-full bg-online border-2 border-surface" />
                </div>
                <div>
                  <p className="text-bright text-sm font-medium">
                    {displayName.trim() || 'Your name here'}
                  </p>
                  <p className="text-dim text-xs">No account needed</p>
                </div>
              </div>
              ...
              <p className="text-center text-dim text-xs">No account required · Free forever</p>
            </form>
          )}
